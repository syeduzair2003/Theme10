...
import MerchantProductsPage from '@/components/Theme-6/comp/MerchantProductsPage';
import OfferDetailsPage from '@/components/Theme-6/comp/OfferDetailsPage';
import CategoryProductOffers from '@/components/Theme-6/comp/CategoryProductOffers';
...
const page = async ({ params }: { params: Props }) => {
    ...
    if (slug.length === 1) {
        ...
        if (productId) {
            // slug[1] has a trailing unique ID → offerDetail page
            ...
            // slug[1] has no trailing ID → it's a category slug
        ...
        // /product/merchant/category/offer-detail
        const merRes = await apiGetMerchantUniqueId(slug[0], companyData.unique_id);
        const merchantId = merRes?.data?.unique_id;
        if (!merchantId) return notFound();
