...
const SidebarRoundMerchantCard = async ({ merchant, merSlug, slugType }: Props) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 scale-50 group-hover:scale-110 opacity-0 group-hover:opacity-100 
        transition-all duration-700 bg-[#800000]/5 rounded-[3rem] blur-3xl"></div>
      ...
      <div className="mt-4 text-center relative z-10">
        <p className="text-[10px] font-black tracking-[0.15em] text-[#4A4A4A] 
          group-hover:text-[#800000] transition-colors duration-300 uppercase leading-tight">
          {merchant?.merchant_name}
        </p>

        {/* Animated Underline */}
        <div className="mx-auto mt-2 h-[1.5px] w-0 bg-[#800000] 
          rounded-full group-hover:w-6 transition-all duration-300"></div>
      </div>
    </Link>
  );
};
