...
const HorizontalBannerSlider = ({
  ...
  return (
    <section
      ...
      className="relative group w-full bg-slate-200"
    >
      {offers.length > 0 ? (
        <>
          <div
            ref={sliderRef}
            className="keen-slider h-[160px] md:h-[220px] lg:h-[280px]"
          >
            {offers.map((item, i) => (
              <div
                key={i}
                className="keen-slider__slide flex items-center justify-center overflow-hidden bg-white"
              >
                <Banner
                  data={item}
                  mer_slug={mer_slug}
