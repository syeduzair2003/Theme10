...
const MobileNavMenu = ({ company_id, mer_slug, slug_type, cat_slug, categories, merchants }: any) => {
    ...
    return (
        <div className="lg:hidden">
            ...
            {open && (
                // Fixed: Added h-screen, w-screen and top-0 left-0 to ensure full coverage
                <div className="fixed inset-0 top-0 left-0 w-screen h-screen bg-[#F5F5DC] z-[9999] flex flex-col animate-in slide-in-from-right duration-500 overflow-hidden">
                    ...
                    <div className="flex items-center justify-between p-6 bg-white border-b border-[#800000]/5 shrink-0">
                        <span className="font-black text-xl tracking-tighter text-[#800000]">MENU</span>
                        <button 
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar bg-[#F5F5DC]">
                        ...
                        <div className="space-y-6">
                            {[
                                ...
                                <div key={group.name} className="bg-white/60 rounded-2xl p-4 border border-white">
                                    <button 
                                    ...
                                    >
                                        <span className={activeTab === group.name ? "text-[#800000]" : ""}>
                                            {group.name}
                                        </span>
                                        <FaChevronDown className={`text-sm transition-transform duration-300 ${activeTab === group.name ? "rotate-180 text-[#800000]" : "text-slate-400"}`} />
                                    </button>
                            ...
                            <div className="px-4 space-y-6 pt-2 pb-10">
                                <Link onClick={() => setOpen(false)} href="/all-products" className="block text-[18px] font-extrabold text-slate-800 no-underline">Products</Link>
                                <Link onClick={() => setOpen(false)} href="/events" className="block text-[18px] font-extrabold text-slate-800 no-underline">Events</Link>
                                <Link onClick={() => setOpen(false)} href="https://blog.gettopdiscounts.com" className="block text-[18px] font-extrabold text-slate-800 no-underline">Blog</Link>
                            </div>
                        </div>
                    </div>
