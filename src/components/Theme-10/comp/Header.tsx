...
const Header = async ({ company_id, domain, mer_slug, slug_type, cat_slug, logo, promotion_slug }: Props) => {
    ...
    const renderDropdown = (name: string, items: any[], getHref: GetHrefFn, iconKey?: string) => (
        <div className="relative group flex items-center h-20" key={name}>
            <Link href={navPaths[name]} className="text-slate-800 hover:text-[#800000] text-[15px] font-bold transition-all flex items-center gap-1.5 no-underline h-full relative">
                {name} <FaChevronDown className="w-2 h-2 opacity-40 group-hover:rotate-180 transition-transform duration-500" />
                
                {/* Maroon Underline for Dropdown Links */}
            ...
            <div className="absolute top-[100%] left-1/2 -translate-x-1/2 w-72 bg-[#FEF9E7] rounded-[2rem] shadow-[0_20px_50px_rgba(128,0,0,0.12)] border border-white/60 opacity-0 invisible group-hover:opacity-100 group-hover:visible translate-y-3 group-hover:translate-y-0 transition-all duration-500 z-50 p-3 overflow-hidden">
                <div className="max-h-[400px] overflow-y-auto no-scrollbar">
                    {items?.map((item: any) => (
                        <Link key={item.id} href={getHref(item)} className="flex items-center gap-3 p-3 rounded-2xl hover:bg-white/60 transition-all no-underline group/item mb-1 border border-transparent hover:border-[#800000]/10">
                            {iconKey && (
                                <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center border border-slate-50 shrink-0 group-hover/item:scale-110 transition-transform shadow-sm">
                                    <Image src={item[iconKey]?.startsWith('http') ? item[iconKey] : `/${item[iconKey]}`} 
                                           alt={item.name || "logo"} width={26} height={26} className="object-contain" />
                                </div>
                            )}
                            <span className="text-sm font-semibold text-slate-700 group-hover/item:text-[#800000]">
            ...
            <div className="max-w-7xl mx-auto px-4 lg:px-10">
                <div className="flex h-20 items-center justify-between gap-6">
                    ...
                    <Link href="/" className="shrink-0 hover:scale-105 transition-transform duration-300">
                        <Image width={140} height={45} src={companyLogo} alt="Logo" className="h-9 w-auto object-contain" priority />
                    </Link>
                        ...
                        {navLinks.map((name) => {
                            ...
                            return (
                                <Link key={name} href={navPaths[name]} 
                                    ...
                                    {name}
                                    <span className={`absolute bottom-5 left-1/2 -translate-x-1/2 h-0.5 bg-[#800000] rounded-full transition-all duration-500 shadow-[0_1px_5px_rgba(128,0,0,0.4)] ${isActive ? "w-6" : "w-0 group-hover:w-6"}`} />
                                </Link>
                            );
                        })}
                    </nav>
