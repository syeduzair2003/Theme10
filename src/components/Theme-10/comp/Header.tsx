...
const Header = async ({ company_id, domain, mer_slug, slug_type, cat_slug, logo, promotion_slug }: Props) => {
    ...
    <div className="absolute top-[100%] left-1/2 -translate-x-1/2 w-72 bg-[#FEF9E7] rounded-[2.5rem] shadow-[0_20px_50px_rgba(128,0,0,0.15)] border border-white/60 opacity-0 invisible group-hover:opacity-100 group-hover:visible translate-y-3 group-hover:translate-y-0 transition-all duration-500 z-50 p-3 overflow-hidden">
        ...
        <div className="max-h-[350px] overflow-y-auto pr-1
            ...
            {items?.map((item: any) => (
                <Link key={item.id} href={getHref(item)} className="flex items-center gap-3 p-3 rounded-2xl hover:bg-white/80 transition-all no-underline group/item mb-1 border border-transparent hover:border-[#800000]/10">
                    {iconKey && (
