...
import { discardHTMLTags, getBaseImageUrl, getCurrencySymbol, getFinalDiscountTag, getMerchantHref, getRandomStoreSeoTitle } from '@/constants/hooks';
...
const OffersCardClient = ({ offer, mer_slug_type, mer_slug, type, productDetailUrl, domain, companyId }: Props) => {
        ...
        <div className="w-full h-full">
            <div className="bg-white rounded-lg border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 p-6 h-full flex flex-col relative">
                ...
                <div className="flex-grow mb-4">
                    ...
                    <ProductDetailsModal
                        product={product}
                        merchantName={offer?.merchant?.merchant_name}
                        merchantLogo={getBaseImageUrl(domain, offer?.merchant?.merchant_logo, "")}
                        domain={domain}
                    />
                </div>
                ...
                <div className="mt-auto">
                    {product?.coupon_code ? (
                        <OfferOutUrl
                            ...
                            domain={domain}
                        >
                            <span className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold text-sm transition-all hover:bg-indigo-700 flex items-center justify-center gap-2 group/btn relative overflow-hidden shadow-md shadow-indigo-100">
                                Show Coupon
                            </span>
                        </OfferOutUrl>
                    ) : (
                        product?.is_detail === 1 && productDetailUrl ? (
                            <Link
                                ...
                                className="w-full bg-slate-900 hover:bg-indigo-600 text-white py-3 rounded-xl font-bold text-sm transition-all duration-300 flex items-center justify-center"
                            >
...
export default OffersCardClient
